...
## 2026-07-06 — M1: unhandled `ValueError` on a loose top-level entry crashed the entire run

A 2026-07-06 round-10 red-team found a live-crash shape: a single top-level `.html` file directly under the export root whose markup merely contains the substring `class="properties"` (e.g. an empty `<table class="properties"><tbody></tbody></table>` — the shape produced when a user exports a single database row as its own standalone HTML export, landing at the export root with no parent entries-folder). `classify_html`'s naive substring match classifies it as a database "entry"; `discover_tree` then groups it into a fake "database" whose `entries_folder` is `src` itself (`hex=None`); `run_conversion` calls `mirror_output_dir(db["entries_folder"].parent, src, out_root)`, i.e. `mirror_output_dir(<one dir ABOVE src>, src, out_root)`; `mirror_output_dir`'s `path.relative_to(src)` raises `ValueError` because that path is outside `src`. Unhandled, this killed the ENTIRE run — no traceback caught, no partial output, no `_conversion_report.md`. Confirmed identically through the packaged CLI's `main()`.

**Fix (contained — `classify_html`'s classification heuristic itself is untouched):** `discover_tree` now detects a database whose `entries_folder` resolves to `src` itself — there is no parent structure for such an "entry" to nest under, so it can never be a real database. Each such entry is folded into the standalone-pages list instead (same fate as any other loose top-level page: written as its own note at the output root, by its own filename) and an explicit warning naming the file is appended to the run's warnings / `_conversion_report.md`. No entry is ever silently dropped, and the crash can no longer happen — `mirror_output_dir` is never called with a path outside `src` for this shape. The rest of the export converts normally and the run exits 0 with a report written.

New test file `test_loose_toplevel_entry.py`: confirms `classify_html` does classify the loose file as `"entry"` (the premise, unchanged); confirms `discover_tree` does NOT leave a fake database rooted at `src` and folds the entry into `pages` with a warning naming the file; confirms `run_conversion` does not raise and still writes `_conversion_report.md`; confirms the rest of the synthetic export (4 databases, 8 entries) still converts normally; confirms the loose entry itself is written as a standalone note (`Loose Row.md`) at the output root. All 5 confirmed CRASHING (`ValueError` from `mirror_output_dir`) or failing against pre-fix code before the fix landed. Full suite grown from 244 to 249.

Queued, not fixed here (M2, unconfirmed): a Notion property literally named `Aliases`/`aliases`/`cssclasses` round-trips verbatim into frontmatter under that literal key — Obsidian treats `aliases`/`cssclasses` as reserved keys, unlike `tags` (which already gets case-insensitive matching + sanitization, `notion_db_to_obsidian.py` ~line 1920). Plausible risk, not verified against a real Obsidian instance; deferred as a separate, larger scoped change (extending the tags-style guard to other reserved keys). See README Known Issues.

---

## 2026-07-06 — L1: fix `no_content_found` false positive for orphan-only directories

K1 (below) added `summary["no_content_found"]`, keyed off only the two HTML-node discovery counts (`n_db_entries == 0 and len(pages) == 0`), computed and printed BEFORE `copy_orphaned_files` runs. But `copy_orphaned_files` unconditionally copies orphaned non-HTML files (PDFs, images, loose attachments) into `out_root` afterward. A directory containing ONLY non-HTML orphan files — a screenshots/downloads folder, or a Notion section exported as PDF instead of HTML — produced `no_content_found = True` plus the "no Notion content found ... valid but empty conversion" warning, WHILE actually copying N files into a new vault. The persisted `_conversion_report.md` then self-contradicted: "Standalone pages written: 0" alongside "Orphaned files copied: N". A library caller trusting the flag to mean "nothing was written" was misled — the exact contract K1 advertised.

**Fix:**
- Moved the `no_content_found` computation (and its warning print) from before the orphan-copy pass to after it, and changed the predicate to `n_db_entries == 0 and len(pages) == 0 and n_orphaned == 0` — using `copy_orphaned_files`'s actual returned count. `no_content_found` is now `True` only when nothing at all was written to `out_root`.
- The warning is now only printed when the corrected predicate is `True` (genuinely nothing written). When orphan files were copied, that's a real, if HTML-less, conversion — not reported as empty; the existing "N orphaned file(s) copied" line already covers it.
- Updated the warning text to mention the orphan count explicitly ("0 database entries, 0 standalone pages, and 0 orphaned files").

An independent adversarial review of the first pass of this fix found a second false-positive shape: `copy_orphaned_files` treats a byte-identical file already on disk as "nothing new to do" and does not count it in its returned copy count (by design — that's what makes re-running the tool against the same src/out pair idempotent). Keying `no_content_found` off that per-run copy count meant a SECOND run against an orphan-only export flipped back to `no_content_found = True`, even though the output vault still held the file the first run copied. Fixed by having `copy_orphaned_files` return a `(copied, present)` tuple: `copied` is this run's new-copy count (unchanged meaning, still what the "N orphaned file(s) copied" console/report line reports); `present` additionally counts files an earlier run already placed. `no_content_found` now keys off `present`, so it reflects "does the output contain orphaned content" rather than "did this invocation do new work."

New tests in `test_run_conversion_input_validation.py`: a directory with only non-HTML orphan files (no `.html` at all) sets `no_content_found = False`, the files land in the output tree, and the "no content found" warning is NOT printed (confirmed failing — `True` + warning printed — against pre-fix code); a second run against the same orphan-only src/out pair still reports `no_content_found = False` (confirmed failing against the first-pass fix, before the `present`-count change); a genuinely empty directory (no files at all) still sets `no_content_found = True` and prints the warning (K1 regression guard); a directory with real HTML entries sets `no_content_found = False` (regression guard). Full suite grown from 240 to 244.

Queued, not fixed here (out of L1's scope, flagged by the same adversarial pass): a genuinely empty source directory still gets a `_conversion_report.md` and a vault-wide `.base` file written into `out_root` even though `no_content_found = True` — so "nothing was written" was never fully literal even before this fix. `no_content_found`'s contract is "no database entries, no standalone pages, no orphaned files" specifically, not "out_root is a literally empty directory"; documented as-is rather than changed, since altering what counts as "content" for this flag is a separate, larger decision (would the `.base`/report themselves need to be gated out too?) than the false-positive this session set out to fix.

Queued, not fixed here: `no_content_found` still keys off DISCOVERED counts, not WRITTEN counts, so it inherits the same "report overstates pre-filter count" limitation as Q1 (a database whose every entry HTML fails `parse_entry` could read `no_content_found == False` while zero notes are written) — noted in the README's Q1 entry, not separately fixed; that's the deferred `parse_entry`-None-drop work.
...
**I2 (MAJOR, H2 over-correction)** — H2's fix for the cross-run "`<Prop> (end)`" collision (`if types_map.get(key) != otype: types_map[key] = otype`) was far broader than that narrow case: it force-overwrote `types_map[key]` for ANY real property whose on-disk type differed from the current run's inference, silently reverting a user's manual Obsidian-UI type customization on EVERY re-run — a worse regression than the rare bug H2 fixed, and one that contradicted the function's own docstring, the `--no-types` help text, the README, and the console "(no existing keys touched)" line, all of which still claimed the old, safer "never clobber existing entries" contract. Restored `if key not in types_map` for the on-disk merge; kept G4's intra-call `real_keys` guard (a real property's type winning over a synthetic `<Prop> (end)` WITHIN one run's own aggregate schema still clobbers nothing on disk — correct, unchanged). The narrow cross-run collision H2 targeted is now a documented, accepted Known Issue instead (see `src/notion_to_obsidian/README.md` Known Issues, row I2) rather than "fixed" by clobbering user data. Verified all 4 contract-text sites (docstring, `--no-types` help, README, console line) already stated the never-clobber contract — they were honest again once the code matched them; no wording changes needed there. `EmitTypesJsonCrossRunRealPropertyWins` rewritten to assert the documented never-clobber outcome for that collision, with a docstring explaining the trade-off; `EmitTypesJsonRealEndPropertyNeverClobbered` (the intra-call G4 tests) unchanged and still green.

**I3 (MEDIUM, resolved by I2)** — the run summary's "no existing keys touched" console line was untrue while H2's force-overwrite stood. After I2 restores never-clobber, verified end-to-end that the on-disk merge path only ever adds genuinely new keys — a re-run with an existing user-customized entry logs a SCHEMA-MERGED entry naming only the run's actually-new keys, never the untouched customized one.

Full suite green throughout, grown from 223 (H-round baseline) to 228.
...
**H2 (Medium-High, G4 cross-run gap)** — `emit_types_json`'s G4 collision guard (precompute real schema keys, don't let a synthetic `<Prop> (end)` key clobber a same-named real property) was intra-call only. Across separate runs onto the SAME output vault, a stale synthetic `<Prop> (end)` entry persisted on disk from a prior run's `types.json` still won over a genuinely real property of that name in a later run, because `key not in types_map` was already False from the stale on-disk entry — `real_keys` only knew the current run's schema, not what's already on disk. `types.json` is documented as an additive, safe-to-rerun cross-run merge, so this was reachable. Fixed: a key that IS a real property in the CURRENT run's schema now force-overwrites `types_map[key]` even if a stale entry already occupies it; only the SYNTHETIC `<Prop> (end)` write keeps the "don't clobber an existing entry" guard. **Trade-off, accepted:** a real property's type is now re-asserted every run rather than "first write wins" — a manual Obsidian-UI type override for a real property is no longer sticky across a re-run onto the same vault. The reviewer's analysis judged this the correct call over leaving cross-run mistyping reachable; taken as-is.

**H3 (LOW, test-adequacy)** — `test_real_end_property_keeps_its_own_type_declared_first` stayed green even with the G4 hunk fully reverted (the original finding) — the pre-existing `if end_key not in types_map` guard already protected that declared-first ordering independent of the `real_keys` precomputation logic it claimed to cover. Re-verified after H2 landed: the test *also* stays green with ONLY the H2 force-overwrite hunk reverted (G4 alone still passes it) — so it gives zero signal for H2 either. The genuinely dependent scenario is the cross-run one (no within-call `real_keys` guard can help there), already covered by the new `EmitTypesJsonCrossRunRealPropertyWins` test. Reworked the vacuous test's docstring to say plainly what it does and doesn't guard, and to point at the test that actually exercises the cross-run force-overwrite path, rather than leave the overclaim in place.

Full suite green throughout, grown from 219 (G-round baseline) to 223.
